...
$sql_agendamentos = "
    SELECT 
        ...
        a.hora AS horario, 
        COALESCE(o.nome, p.nomeCompleto) AS nome_oficina
    FROM 
        agendamento a 
    JOIN 
    ...
    LEFT JOIN 
        prestadordeservico p ON p.idPrestador = aso.id_Oficina 
    WHERE 
        a.id_Usuario = ?
";
...
$sql_orcamentos = "SELECT
                        ...
                        GROUP_CONCAT(DISTINCT s.descricao SEPARATOR ', ') AS descricao_servico,
                    ...
                    GROUP BY
                        o.idOrcamento, o.dataOrcamento, o.validadeOrcamento, of.idOficina, of.nome
                    ORDER BY
                        o.idOrcamento DESC";
        ...
        <?php
            if ($result_orcamentos->num_rows > 0) {
                ...
                while ($row = $result_orcamentos->fetch_assoc()) {
                    if (!in_array($row['numero_orcamento'], $orcamentos_exibidos)) {
                        echo "<tr>
                                <td>{$row['numero_orcamento']}</td>
                        ...
                        $orcamentos_exibidos[] = $row['numero_orcamento'];
                    }
                }
            } else {
...
<div class="button-container">
    <button onclick="window.location.href='../../login/admFisica.php'">Voltar</button>
</div>
